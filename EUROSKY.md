...
| Relay: `host-throttled` accounts are no longer stranded on a host with room for them: re-evaluated on host move, upstream reactivation and dropped events; `changeLimits` promotes up to the free room and can be re-run; separate `host-throttled` label on `events_warn_counter` (EUR-280, EUR-344) | see git log | `cmd/relay/relay/{account,host,ingest,throttle_test}.go` | Ours only; to go upstream |
