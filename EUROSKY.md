...
| Fork CI: multi-arch GHCR image builds with branch-prefixed tags and git tags for hepa, relay and mock-csam; upstream sync workflow; builds triggered from `eurosky`                      | `8d68152d2`, `608b490c`, `b906e2c4`, this change   | `sync-upstream.yaml`, `container-{hepa,relay,mock-csam}-ghcr.yaml`, `golang.yml`                                                                                                                                                                                                     | Ours only                                                         |

Known breakage to fix in the first sync: `gen/main.go` imports `atproto/data` and `atproto/label`, which upstream renamed to `atproto/atdata` and `atproto/labeling` before our base, so `go build ./...` and `make lint` fail on it (`make build` and `make test` don't compile that package).
